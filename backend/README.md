# Marketing Engine OS - Backend (Flask)

Ten README ma przede wszystkim pomóc w uruchomieniu projektu.

## Jak uruchomiæ aplikacjê

## A) Pierwsze uruchomienie (jednorazowo)

1. Zainstaluj wymagania systemowe:
- Python `3.10+`
- Node.js `18+` + `npm`
- `ffmpeg` w `PATH` (`ffmpeg -version`)

2. Przygotuj backend:

```bash
cd c:\Development\ContentSaaS\backend
python -m venv .venv
.\.venv\Scripts\activate
pip install -r requirements.txt
```

3. Przygotuj frontend (z katalogu g³ównego projektu):

```bash
cd c:\Development\ContentSaaS
npm install
```

4. (Opcjonalnie) utwórz `backend/.env`:

```env
PORT=5000
GEMINI_MODEL=gemini-3.1-flash-lite-preview
```

`GEMINI_API_KEY` mo¿esz ustawiæ póŸniej z UI w zak³adce `Silnik AI`.

## B) Ka¿de kolejne uruchomienie (codzienny start)

Uruchom 2 terminale.

Terminal 1 - backend:

```bash
cd c:\Development\ContentSaaS\backend
.\.venv\Scripts\activate
python server.py
```

Terminal 2 - frontend:

```bash
cd c:\Development\ContentSaaS
npm run dev
```

## C) Konfiguracja klucza Gemini

Po uruchomieniu aplikacji:

1. WejdŸ na `http://localhost:5173`.
2. Otwórz zak³adkê `Silnik AI`.
3. Wklej klucz API Gemini i wybierz model.
4. Zapisz konfiguracjê.

Frontend wyœle ustawienia do backendu przez `POST /api/ai/config`.

## D) Sprawdzenie, czy dzia³a

- Frontend: `http://localhost:5173`
- Backend health: `http://localhost:5000/api/health`

Przyk³adowy test:

1. W `Maksymalizatorze Treœci` wklej URL YouTube.
2. Uruchom generowanie.
3. Pobierz 1 klip (sprawdza `yt-dlp` + `ffmpeg`).

## Najczêstsze problemy

- `ModuleNotFoundError`:
SprawdŸ, czy aktywne jest `backend/.venv` i czy by³ wykonany `pip install -r requirements.txt`.

- `ffmpeg not found`:
Dodaj `ffmpeg` do `PATH` i uruchom terminal ponownie.

- Frontend dzia³a, backend "milczy":
SprawdŸ `GET /api/health` i port `5000`.

- B³êdy Gemini (`503`, quota, invalid key):
Zmieñ model w `Silnik AI`, sprawdŸ klucz i limity API.

## Co robi backend

Backend odpowiada za:

- transkrypty YouTube,
- generowanie AI (`clips`, `blog`, `social`, `business growth strategy`),
- asynchroniczne pobieranie klipów,
- zapis danych projektów/skryptów/transkryptów do JSON.

Kluczowe pliki:

- `server.py`
- `transcript_fetcher.py`
- `content_processor.py`
- `business_growth_strategy_processor.py`
- `download_clip.py`
- `database.py`

## Dalsza dokumentacja

- Kontrakty API: `backend/docs/backend_api.md`
- Przewodnik frontendu: `backend/docs/frontend_guide.md`
- Szczegó³owy setup: `backend/docs/setup_guide.md`


